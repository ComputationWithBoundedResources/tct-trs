...
instance (Ord f, PP.Pretty f) => PP.Pretty (UsablePositions f) where
  pretty up@(UP m)
    ...
    | otherwise = PP.vsep $ PP.punctuate (PP.char ',') (map ppsym syms)
    where
      syms = M.keys $ M.filter (not  . IS.null) m
      ppsym sym = PP.text "Uargs" <> PP.parens (PP.pretty sym) <> PP.text " = " <> PP.set [ PP.int i | i <- usablePositionsOf up sym ]
...
usableArgsWhereApplicable prob onlyCompound useUA = case (onlyCompound, useUA, Prob.startTerms prob) of
  (True, True, _)               -> restrictToSignature compSig (usableArgs str trs) `union` emptyWithSignature nonCompSig
  (True, _,    _)               -> fullWithSignature compSig `union` emptyWithSignature nonCompSig
