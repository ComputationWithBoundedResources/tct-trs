...
data MatrixInterpretation fun var a =
...
data MatrixInterpretationEntry fun
  ...
  | MIConstZero -- Introducing constants here circumvents the introduction of a RingConst class like in tct2.
                -- Implicitly we have a Semiring for Matrix Interpretations. So we need zero (and one).
  | MIConstOne
...
data MatrixKind fun
  = UnrestrictedMatrix
  | ConstructorBased (Set.Set fun) (Maybe Int)
  | TriangularMatrix (Maybe Int)
  | ConstructorEda (Set.Set fun) (Maybe Int)
  | EdaMatrix (Maybe Int)
  deriving (Show)
...
pprintLInter :: (Eq a, PP.Pretty a, SR.SemiRing a) => String -> Int -> (var -> PP.Doc) -> LinearInterpretation var a -> PP.Doc
...
-- what does EDA stand for? -- e... deterministic automaton?
